var ctx = getContext()// com.mojang.minecraftpe.MainActivity.currentMainActivity.get();
...
var debug_mode = true, debug_folder = sdcard + "/1-Work-Space-1-JavaScript/music player/";
...
function setAppTheme(id){try{
	ctx.setTheme(id);
}catch(e){err_dialog(e);}}
if (debug_mode)
{
	//ctx.setRequestedOrientation(android.content.pm.ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
	//new java.lang.Thread(ctx).sleep(1000);
}
if (debug_mode)
	print("finished loading");
	
setAppTheme(android.R.style.Theme_Holo_Light_DarkActionBar);
